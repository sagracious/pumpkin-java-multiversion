...
The 1.21.11 to 1.21.9 step converts world-border interpolation time from seconds to ticks, drops the new nautilus breathing effect, tracks game time for wolf/bee anger metadata, and maps Nautilus, Zombie Nautilus, Camel Husk, and Parched spawns and tags to older entity types. Nautilus-only metadata is filtered, Camel Husk fields map onto Camel metadata, and entity hover names use the same stand-ins. The recipe book now translates the slot-display shapes emitted by Pumpkin's 26.3 recipe writer (empty, any-fuel, item, item-stack and composite) plus crafting item holder sets for clients from 1.21.2 through 26.2. Unknown slot-display codecs fail closed, and recipe-book packets are still dropped for older clients. Via's other slot-display variants, item-component backup/restore, and other entity, particle, registry, and tag behavior remain incomplete.
