...
The 1.21.11 to 1.21.9 step converts world-border interpolation time from seconds to ticks, drops the new nautilus breathing effect, tracks game time for wolf/bee anger metadata, and maps Nautilus, Zombie Nautilus, Camel Husk, and Parched spawns and tags to older entity types. Nautilus-only metadata is filtered, Camel Husk fields map onto Camel metadata, and entity hover names use the same stand-ins. Recipe displays now translate all 11 slot-display codecs emitted by Pumpkin's 26.3 recipe writer in `RECIPE_BOOK_ADD`, `UPDATE_RECIPES`, and `PLACE_GHOST_RECIPE` for clients from 1.21.2 onward. Slot and recipe item IDs are mapped with Via's identity fallback, explicit item holder sets are rewritten, and 26.3 tag-display holder sets downgrade to the older string form. Recipe-display packets are dropped for clients before 1.21.2; unknown codecs fail closed. Via's other item-component backup/restore paths, and other entity, particle, registry, and tag behavior remain incomplete.
